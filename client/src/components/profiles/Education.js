...
const Education = ({education}) => {
        ...
        const educations = education.map((educ, i) => (
                <div key={educ._id} className="mb-3 mt-4">
                        ...
                        <p className="mb-2"><Moment format="YYYY/MM/DD">{educ.from}</Moment> - {
                                educ.to === null ? (' Now') : (<Moment format="YYYY/MM/DD">{educ.to}</Moment>)
                        }</p>
                        <p className="mb-2"><strong>Degree: </strong>{educ.degree}</p>
                        <p className="mb-2"><strong>Field of study: </strong>{educ.fieldofstudy}</p>
                        <p className="mb-2">
